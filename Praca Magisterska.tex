...
\begin{document}
	...
	\begin{definition}[słaba preferencja]
		...
		\iff
		a \overset{w}{\succ} b \ \text{lub}\ a \sim b.
		\]
	...
	W dalszej części pracy, dla przejrzystości zapisu, będziemy czasem posługiwać się takim przedstawieniem profilu preferencji, w którym profil opisuje liczebność grup wyborców posiadających daną relację preferencji.
	...
	\begin{definition}[metoda decyzyjna]
		...
		Metodę wyborczą \(M\) nazywamy decyzyjną, jeżeli dla każdego profilu preferencji wskazuje dokładnie jednego zwycięzcę, tj.
		\[
		M : \Sigma \to K.
		\]
	\end{definition}
	...
	\begin{definition}[zwycięzca Kemeny'ego]
		Niech \(\succ\in \mathcal{R}\) będzie rankingiem Kemeny’ego, tj. relacją preferencji maksymalizującą funkcję
		\[
		\sum_{w \in W} \#\{(a,b)\in K \times K \;:\; a \succ b \ \text{oraz}\ a \overset{w}{\succ} b\}.
		\]
		
		Zbiór wszystkich rankingów Kemeny’ego oznaczamy przez \(\mathcal{K}(R)\).
		
		Kandydata \(a \in K\) nazywamy zwycięzcą Kemeny’ego, jeżeli istnieje ranking \(\succ\in \mathcal{K}(R)\), w którym \(a\) zajmuje pierwsze miejsce.
	\end{definition}
	...
	\begin{example}
		...
		W istocie dla każdego rankingu \(\succ\) zachodzi:
		\[
