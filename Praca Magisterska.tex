...
\begin{document}
	...
	W niniejszym rozdziale wprowadzamy podstawowe oznaczenia oraz pojęcia z zakresu teorii wyboru, które będą wykorzystywane w dalszej części pracy. Formalizacja ta umożliwi precyzyjne sformułowanie oraz analizę paradoksu nieobecności.
	
	Niech \(K\) oznacza zbiór wszystkich kandydatów, przy czym \(\#K = m\), natomiast \(W\) oznacza zbiór wszystkich wyborców, gdzie \(\#W = n\).
	
	Przez
	\[
	\mathcal{E}(W) := \mathcal{P}(W) \setminus \{\emptyset\}
	\]
	oznaczamy zbiór wszystkich elektoratów, tj. zbiór wszystkich niepustych podzbiorów zbioru \(W\). Elementy zbioru \(\mathcal{E}(W)\) interpretujemy jako grupy wyborców biorących udział w głosowaniu.
	
	\begin{definition}[Relacja preferencji]
		Relacją preferencji na zbiorze \(K\) nazywamy zupełną, antysymetryczną oraz przechodnią relację binarną określoną na \(K\).
	\end{definition}
	
	Relacja preferencji opisuje indywidualne upodobania wyborcy względem kandydatów. Zupełność oznacza, że każda para kandydatów jest porównywalna, antysymetryczność wyklucza sprzeczne preferencje, natomiast przechodniość zapewnia ich logiczną spójność.
	...
	\begin{definition}[Przestrzeń relacji preferencji]
		Zbiorem wszystkich relacji preferencji na zbiorze \(K\) nazywamy zbiór
		\[
		\mathcal{R}.
		\]
	\end{definition}
	
	Dla każdego wyborcy \(w \in W\) zachodzi \(\overset{w}{>} \in \mathcal{R}\).
	
	Dla zwięzłości zapisu, pomijając indeks wyborcy, ciąg symboli
	\[
	abcd
	\]
	...
	\begin{definition}[Słaba preferencja]
		Dla relacji ścisłej \(\overset{w}{>}\) definiujemy odpowiadającą jej relację słabej preferencji \(\overset{w}{\geq}\) wzorem
		\[
		a \overset{w}{\geq} b
		\iff
	...
	Niech \(V \in \mathcal{E}(W)\) będzie ustalonym elektoratem.
	
	\begin{definition}[Profil preferencji]
		Profilem preferencji na elektoracie \(V\) nazywamy funkcję
		\[
		R : V \to \mathcal{R},
		\]
		która każdemu wyborcy przyporządkowuje jego relację preferencji.
	\end{definition}
	
	Zbiór wszystkich profili preferencji określonych na elektoracie \(V\) oznaczamy przez
	\[
	\mathcal{R}^{V}.
	\]
	
	Dla pewnej grupy wyborców będziemy korzystać z przedstawień za pomocą tabeli
	...
	gdzie górny wiersz oznacza liczbę wyborców z daną poniżej relacją preferencji.
	...
	\begin{definition}[Model wyborczy]
		...
		\Sigma := (V, R),
		\]
		gdzie \(V \in \mathcal{E}(W)\) jest elektoratem, natomiast \(R \in \mathcal{R}^{V}\) jest profilem preferencji.
	\end{definition}
	...
	W dalszej części pracy będziemy analizować wpływ udziału wyborców w głosowaniu na wynik wyborów. W tym celu wprowadzamy operacje dodawania oraz usuwania wyborców z rozważanego elektoratu.
	
	Niech \(R \in \mathcal{R}^{V}\) będzie profilem preferencji, \(w_i \in V\) oraz \(w_j \in W \setminus V\). Dla zadanych relacji preferencji \(\overset{w_i}{>}, \overset{w_j}{>} \in \mathcal{R}\) definiujemy:
	\[
	R - w_i := R \setminus \{(w_i, \overset{w_i}{>})\},
	\]
	\[
	R + (w_j, \overset{w_j}{>}) := R \cup \{(w_j, \overset{w_j}{>})\}.
	\]
	
	Operacja \(R - w_i\) oznacza usunięcie wyborcy z elektoratu, natomiast \(R + (w_j, \overset{w_j}{>})\) oznacza dodanie nowego wyborcy wraz z jego preferencjami.
	
	Jeżeli dodajemy lub usuwamy kilku wyborców o identycznych preferencjach, stosujemy skrócony zapis:
	\[
	R + k \cdot \overset{w}{>},
	\qquad
	R - k \cdot \overset{w}{>}.
	...
	Wprowadzone operacje pozwalają analizować zależność wyniku wyborów od składu elektoratu. W szczególności umożliwiają badanie sytuacji, w których uczestnictwo wyborcy w głosowaniu prowadzi do wyniku mniej dla niego korzystnego niż jego nieobecność.
	\newpage
	...
	\begin{definition}[Kandydat Condorceta]
		Niech \(R \in \mathcal{R}^{V}\) będzie profilem preferencji. Kandydat \(a \in K\) jest kandydatem Condorceta (zwycięzcą Condorceta) wtedy i tylko wtedy, gdy w bezpośrednim porównaniu większościowym wygrywa z każdym innym kandydatem, tzn.
		\[
		\forall b \in K,\ b \neq a \quad
	...
	\begin{definition}[Kryterium Condorceta]
		Metoda wyborcza \(M\) spełnia kryterium Condorceta wtedy i tylko wtedy, gdy dla każdego profilu preferencji \(R\), jeżeli istnieje kandydat Condorceta \(a \in K\), to kandydat \(a\) jest jedynym zwycięzcą wyłonionym przez metodę \(M\), tzn.
		\[
		M(R) = \{a\}.
		\]
	\end{definition}
	...
	\begin{definition}[Przegrany Condorceta]
		Niech \(R \in \mathcal{R}^{V}\). Kandydat \(a \in K\) jest przegranym Condorceta, jeżeli przegrywa z każdym innym kandydatem w głosowaniach parami, tzn.
		\[
		\forall b \in K,\ b \neq a \quad
		\#\{w \in V : b \overset{w}{>} a\}
		>
	...
	\begin{definition}[Kryterium obecności]
		Metoda wyborcza \(M\) spełnia kryterium obecności, jeżeli każdy wyborca słabo preferuje udział w głosowaniu względem rezygnacji z udziału, tzn.
		\[
	...
	\begin{definition}[Zwycięzca Maximina]
		Kandydata \(a \in K\) nazywamy zwycięzcą metody Maximina, jeżeli maksymalizuje wartość funkcji
		\[
	...
	\begin{definition}[Ranking Kemeny'ego]
		Rankingiem Kemeny'ego nazywamy taką relację preferencji \(>\,\in \mathcal{R}\), która maksymalizuje zgodność z preferencjami wyborców, tj. maksymalizuje wartość
		\[
	...
	\begin{definition}[Zwycięzca Kemeny'ego]
		Kandydat \(a \in K\) jest zwycięzcą Kemeny'ego wtedy i tylko wtedy, gdy zajmuje pierwsze miejsce w pewnym rankingu Kemeny'ego.
	\end{definition}
	...
	\section{Silny paradoks nieobecności}
	...
	\section{Załączniki}
	\subsection{Kod źródłowy symulacji}
	\subsection{Dodatkowe obliczenia}
