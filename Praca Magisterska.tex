...
\begin{document}
	...
	\begin{definition}[Ranking i zwycięzca Kemeny’ego]
		...
		Kandydata \(a \in K\) nazywamy zwycięzcą Kemeny’ego, jeżeli istnieje ranking
		\[
		\succ \in \mathcal K(R),
		\]
	...
	\begin{example}
		Rozważmy profil preferencji \(R\) dany przez tabelę:
		...
		Policzmy marginesy większościowe:
		\[
		g_R(a,b)=3,
		\qquad
		g_R(b,c)=3,
		...
		Otrzymujemy więc cykl Condorceta:
		\[
		a \succ_M b,
		\qquad
		b \succ_M c,
		...
		Aby wyznaczyć ranking Kemeny’ego, obliczamy dla każdego rankingu sumę marginesów zgodnych z tym rankingiem.
		...
		Z kolei ranking przeciwny
		\[
		c \succ b \succ a
		\]
		otrzymuje wartość przeciwną:
		\[
		-5.
		\]
		
		W istocie dla każdego rankingu \(>\) zachodzi:
		\[
		\operatorname{score}(>^{-1})
		=
		-\operatorname{score}(>),
		\]
		gdzie \(>^{-1}\) oznacza ranking odwrotny.
		
		Wystarczy więc analizować jedynie połowę wszystkich rankingów.
		
		W przedstawionym przykładzie największą wartość uzyskuje ranking
		\[
		a \succ b \succ c,
		\]
		a zatem jest on rankingiem Kemeny’ego.
		
		W konsekwencji jedynym zwycięzcą Kemeny’ego jest kandydat
		\[
		a.
		\]
		
		Intuicyjnie metoda Kemeny’ego stara się zachować możliwie najwięcej silnych relacji większościowych. W tym przypadku relacje
		\[
		a \succ_M b
		\qquad \text{oraz} \qquad
		b \succ_M c
		\]
		mają większy margines niż relacja
		\[
		c \succ_M a,
		\]
		dlatego ranking Kemeny’ego „łamie” jedynie tę ostatnią relację.
	\end{example}
