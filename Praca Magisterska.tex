...
\begin{document}
	...
	\begin{proof}
		Załóżmy teraz, że zbiór \(K\) zawiera co najmniej czterech kandydatów oraz że metoda \(M\) spełnia kryterium Condorceta. Najpierw pokażemy, że jeżeli zbiór potencjalnych wyborców \(W\) zawiera co najmniej \(37\) wyborców, to metoda \(M\) nie spełnia pozytywnego kryterium obecności. Następnie pokażemy, że jeżeli \(W\) zawiera co najmniej \(34\) wyborców, to metoda \(M\) nie spełnia negatywnego kryterium obecności.
		
		Dowód pierwszej części opiera się na argumencie przedstawionym przez Moulin \cite{moulin1988}.
		
		Przechodzimy teraz do dowodu drugiej części.
	\end{proof}
	...
	\begin{proof}
		...
		Rozważmy profil sześciu wyborców \(R\) przedstawiony w Tabeli~\ref{fig:profil_R}. (Liczby nad każdą kolumną opisują, ile razy dana relacja preferencji występuje w profilu \(R\). Na przykład w \(R\) dokładnie dwóch wyborców ma ranking \(bdca\).)
